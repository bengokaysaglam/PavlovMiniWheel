...
class BallToGoal(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(f"[DEBUG] Subscribing to: ball_point={ball_point_topic}, visible={ball_visible_topic}")
        ...
        self.get_logger().info(f"[DEBUG] Timer created: period={timer_period:.3f}s (rate={publish_rate_hz} Hz)")
    ...
    def _visible_cb(self, msg: Bool) -> None:
        ...
        self.get_logger().info(f"[DEBUG] Visible callback: ball_visible={self._ball_visible}")
    ...
    def _ball_point_cb(self, msg: PointStamped) -> None:
        ...
        self.get_logger().info(f"[DEBUG] Ball point callback: x={msg.point.x:.3f}, y={msg.point.y:.3f}")
    ...
    def _tick(self) -> None:
        self.get_logger().info("[DEBUG] _tick() called")
        ...
        self.get_logger().info(f"[DEBUG] ball_fresh={ball_fresh}, visible_fresh={visible_fresh}, ball_visible={self._ball_visible}, last_ball_point={self._last_ball_point is not None}")
        ...
        self.get_logger().info(f"[DEBUG] Ball point: x={ball_x:.3f}, y={ball_y:.3f}")
        ...
        if not math.isfinite(ball_x) or not math.isfinite(ball_y):
            self.get_logger().warn(f"[DEBUG] Ball coordinates not finite: x={ball_x}, y={ball_y}")
            return

        if ball_x < min_ball_x_m:
            self.get_logger().warn(f"[DEBUG] Ball x={ball_x:.3f} is too close (min={min_ball_x_m:.3f})")
            return
